...
function ReservationRows({ sortedReservations, date }) {
  ...
  sortedReservations.forEach((res, index) => {
    ...
    if (res.reservation_date === date && status !== "finished") {
  ...
  return (
    <tbody>
      <tr>
        <td className="text-center mt-5 fs-1">No reservations for {date}.</td>
      </tr>
    </tbody>
  );
}
